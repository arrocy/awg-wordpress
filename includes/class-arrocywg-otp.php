...
class ArrocyWG_OTP {
    public function __construct() {
        ...
        add_action('wp_ajax_arrocywg_send_otp', [$this, 'ajax_send_otp']);
        add_action('wp_ajax_nopriv_arrocywg_send_otp', [$this, 'ajax_send_otp']);
    }
    ...
    public function add_login_otp_fields() {
        ?>
        <p class="form-row form-row-wide">
            <label for="arrocywg_login_otp">OTP <span class="required">*</span></label>
            <input type="text" name="arrocywg_login_otp" id="arrocywg_login_otp" required />
        </p>
        <?php
    }
    ...
    public function add_reset_otp_fields() {
        ?>
        <p>
            <label for="arrocywg_reset_otp">OTP <span class="required">*</span></label>
            <input type="text" name="arrocywg_reset_otp" id="arrocywg_reset_otp" required />
        </p>
        <?php
    }

    public function ajax_send_otp() {
        if (get_transient("arrocywg_otp_sent_{$phone}")) {
            wp_send_json_error('Please wait before requesting another OTP.');
        }
        set_transient("arrocywg_otp_sent_{$phone}", true, 60); // 1-minute cooldown

        $phone = sanitize_text_field($_POST['phone'] ?? '');
        if (!$phone) wp_send_json_error('Missing phone');

        $otp = rand(100000, 999999);
        set_transient("arrocywg_otp_{$phone}", $otp, 10 * MINUTE_IN_SECONDS);

        ArrocyWG_API::send($phone, "Your OTP is: {$otp}");
    ...
    public function validate_otp($username, $email, $errors) {
        ...
        $stored = get_transient("arrocywg_otp_{$phone}");
    ...
    public function validate_login_otp($user, $password) {
        ...
        delete_transient("arrocywg_otp_{$phone}");
        return $user;
    }
