...
class ArrocyWG_API {
    ...
    public static function send_woocommerce_event_message($order_id, $event_key) {
        if (!get_option("arrocy_wg_template_{$event_key}_enabled")) return;

        $order = wc_get_order($order_id);
        $phone = $order->get_billing_phone();
        $message = get_option("arrocy_wg_template_{$event_key}_message", "Your order update: {$event_key}");
    ...
    public static function send_easydigitaldownloads_event_message($payment_id, $event_key) {
        ...
        $message = get_option("arrocy_wg_template_{$event_key}_message", "Your payment update: {$event_key}");
    ...
    public static function send_wpforms_event_message($fields, $entry, $form_data, $entry_id, $event_key) {
        ...
        $message = get_option("arrocy_wg_template_{$event_key}_message", "Your form submission update: {$event_key}");
    ...
    public static function send_learndash_event_message($data, $user_id, $event_key) {
        ...
        $message = get_option("arrocy_wg_template_{$event_key}_message", "Your LearnDash update: {$event_key}");
    ...
    public static function send_memberpress_event_message($subscription_id, $event_key) {
        ...
        $phone = get_user_meta($user_id, 'phone', true);
        $message = get_option("arrocy_wg_template_{$event_key}_message", "Your MemberPress update: {$event_key}");
    ...
    public static function send_gravityforms_event_message($entry, $form) {
        $event_key = 'gravityforms_submitted';
        if (!get_option("arrocy_wg_template_{$event_key}_enabled")) return;

        $phone = rgar($entry, 'phone');
        $message = get_option("arrocy_wg_template_{$event_key}_message", "Your Gravity Forms submission update: {$event_key}");
    ...
    public static function notify_admins($message) {
        $admin_phones = get_option('arrocy_wg_admin_phones');
        if ($admin_phones) {
    ...
    public static function send($phone, $message) {
        $api_url = get_option('arrocy_wg_url');
        $token   = get_option('arrocy_wg_token');
